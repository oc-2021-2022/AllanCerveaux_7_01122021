      ...
      <article class="recipe col-4 my-3" data-id="${id}">
        <div class="card">
          ...
          <div class="card-body overflow-hidden flex-grow-1">
            <div class="row">
              <ul class="list-group list-group-flush col-6">${ingredients.map(({ingredient, quantity, unit}) => /* html */`<li class="list-group-item bg-transparent border-0 py-1"><strong>${ingredient}</strong> : ${quantity ?? ''} ${unit ?? ''}</li>`).join('')}</ul>
